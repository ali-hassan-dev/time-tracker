            ...
            <table class="min-w-full bg-white border border-gray-300 shadow-lg rounded-lg">
                <thead class="bg-gray-200">
                    <tr>
                        ...
                        <th class="py-3 px-4 border-b text-center">Start Time</th>
                        <th class="py-3 px-4 border-b text-center">End Time</th>
                ...
                <tbody>
                    @foreach ($timeLogs as $key => $timeLog)
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="py-3 px-4 border-b text-center">{{ $timeLog->start_time ?? '-' }}</td>
                            <td class="py-3 px-4 border-b text-center">{{ $timeLog->end_time ?? '-' }}</td>
                            ...
                            <td class="py-3 px-4 border-b text-center">
                                <a href="{{ route('admin.users.time-logs.edit', ['user' => $user->id, 'timeLog' => $timeLog->id]) }}"
                                    class="text-blue-500 hover:underline">Edit</a>
                                <form
                                    action="{{ route('admin.users.time-logs.destroy', ['user' => $user->id, 'timeLog' => $timeLog->id]) }}"
